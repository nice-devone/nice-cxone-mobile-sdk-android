 ...
 * @param coroutineScope [CoroutineScope] used for launching suspend event calls.
 */
class AnalyticsHandler(
     ...
     */
    fun sendConversion(type: String, amount: Double) {
        events?.let { handler ->
            coroutineScope.launch {
                try {
                    handler.conversion(type, amount)
                } catch (cancelled: CancellationException) {
                    throw cancelled
                } catch (logged: CXoneException) {
                    warning("Failed to send conversion event", logged)
                }
            }
        }
    }
    ...
    private fun sendPageView(pageInfo: PageInfo) = scope("sendPageView") {
        info("sendPageView(title=${pageInfo.title}, url=${pageInfo.url})")
        events?.let { handler ->
            coroutineScope.launch {
                try {
                    handler.pageView(pageInfo.title, pageInfo.url)
                } catch (cancelled: CancellationException) {
                    throw cancelled
                } catch (logged: CXoneException) {
                    warning("Failed to send pageView event", logged)
                }
            }
        }
    ...
    private fun sendPageViewEnded() = scope("sendPageViewEnded") {
        ...
        if (pageViewEndedNeeded) {
            ...
            events?.let { handler ->
                coroutineScope.launch {
                    try {
                        handler.pageViewEnded(last.title, last.url)
                    } catch (cancelled: CancellationException) {
                        throw cancelled
                    } catch (logged: CXoneException) {
                        warning("Failed to send pageViewEnded event", logged)
                    }
                }
            }
