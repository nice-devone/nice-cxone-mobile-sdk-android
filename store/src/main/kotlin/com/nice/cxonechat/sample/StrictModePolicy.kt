...
internal object StrictModePolicy {
    ...
    private val threadPolicy = RuleBasedPenalty(
        // AppAuth reads auth_config.json from res/raw on first use
        allow(
            allOf(
                violation(DiskReadViolation::class),
                classNamed(AuthConfiguration::class.qualifiedName!!, "from"),
            )
        ),
        // AppAuth internally reads disk when binding to CustomTabs service
        allow(
            allOf(
                violation(DiskReadViolation::class),
                classNamed(PatternMatcher("net.openid.appauth.", PatternMatcher.PATTERN_PREFIX))
            )
        ),
        // Chrome Custom Tabs performs a disk read (File.exists check) during the Binder
        // handshake when AppAuth binds to it for the OAuth browser session.
        allow(
            allOf(
                violation(DiskReadViolation::class),
                classNamed(PatternMatcher("org.chromium.", PatternMatcher.PATTERN_PREFIX))
            )
        ),
        // DE-117407
        allow(
        ...
        // FCM is launching an unsafe intent on Android 16
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.BAKLAVA) {
            allow(
                allOf(
                    violation(UnsafeIntentLaunchViolation::class),
                    classNamed("com.google.android.gms.common.api.internal.GoogleApiManager", "handleMessage")
                )
            )
        } else {
            null
        },
        @Suppress("KotlinConstantConditions", "RedundantSuppression") // Build time variant
    ...
    private val vmPolicy = RuleBasedPenalty(
        // AppAuth's AuthorizationManagementActivity relaunches the browser intent on resume,
        // which Android 12+ VM policy flags as UnsafeIntentLaunchViolation. This is internal
        // to AppAuth and cannot be fixed from the app side.
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            allow(
                allOf(
                    violation(UnsafeIntentLaunchViolation::class),
                    classNamed("net.openid.appauth.AuthorizationManagementActivity", "onResume")
                )
            )
        } else {
            null
        ...
        // AppAuth token exchange uses HttpURLConnection internally without socket tagging;
        // cannot be fixed from the app side since AppAuth doesn't expose its executor.
        allow(
            allOf(
