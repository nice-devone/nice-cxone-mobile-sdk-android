...
internal object GroupStateTestUtils {
    ...
    fun createTestMessage(status: MessageStatus, sender: Person? = null, createdAt: Instant = System.now()): Text {
        return Text(
            UiSdkText(
                text = nextString(),
    ...
    fun createTestEmojiMessage(status: MessageStatus, sender: Person? = null, createdAt: Instant = System.now()): EmojiText {
        return EmojiText(
            UiSdkText(
                text = "😀",
                metadata = UiSdkMetadata(status = status),
