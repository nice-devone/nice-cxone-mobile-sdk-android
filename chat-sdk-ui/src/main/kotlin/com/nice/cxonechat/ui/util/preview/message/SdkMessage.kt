...
internal data class Text(
    ...
    override val isAiGenerated: Boolean = false,
) : SdkText()
...
internal data class ListPicker(
    ...
    override val actions: Iterable<Action> = listOf(
        ...
        ReplyButton(
            text = "Reply with a cat image",
            mediaUrl = "https://http.cat/203",
            mediaMimeType = "image/jpeg",
            description = "Some long description to go with the image"
        )
    ),
    override val id: UUID = UUID.randomUUID(),
    override val threadId: UUID = UUID.randomUUID(),
...
internal data class TimePicker(
    ...
    override val timeSlots: Iterable<TimeSlot> = defaultTimeSlots(),
    override val id: UUID = UUID.randomUUID(),
    override val threadId: UUID = UUID.randomUUID(),
    override val createdAt: Instant = Clock.System.now(),
    override val direction: MessageDirection = ToClient,
    ...
    companion object {
        fun defaultTimeSlots(): List<TimeSlot> {
            val now = Clock.System.now()
            return listOf(
                TimePickerSlot(startTime = now - 24.hours),
                TimePickerSlot(startTime = now - 25.hours),
                TimePickerSlot(startTime = now),
                TimePickerSlot(startTime = now + 30.minutes),
                TimePickerSlot(startTime = now + 60.minutes),
                TimePickerSlot(startTime = now + 90.minutes),
            )
        }
    }
}
...
internal data class QuickReply(
    ...
    override val actions: Iterable<Action> = listOf(
        ReplyButton(text = "Some text", description = "Some long description to go with it"),
