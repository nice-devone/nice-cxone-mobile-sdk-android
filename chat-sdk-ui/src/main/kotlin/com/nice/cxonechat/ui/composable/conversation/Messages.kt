...
internal fun ColumnScope.Messages(
    ...
    LazyColumn(
        ...
        // Typing indicator (should be read first)
        if (agentIsTyping && agentDetails != null) {
            item(key = "TypingIndicator", contentType = Typing) {
        ...
        val totalSections = groupedMessages.size
        groupedMessages.forEachIndexed { sectionIndex, section ->
            // Reverse section order for accessibility
            val reversedSectionIndex = totalSections - sectionIndex
            val sectionBase = reversedSectionIndex * 100f
            itemsIndexed(
                ...
                val reverseIndex = section.messages.size - messageIndex
                val isLast = messageIndex == section.messages.lastIndex
                val sender = message.sender
                val groupState = remember(section, messageIndex, sender, isLast) {
                    getGroupState(section, messageIndex)
                }
                val position = remember(sectionIndex, messageIndex) { Position(sectionIndex, messageIndex) }
                ...
                val messageTraversal = sectionBase + 1f + reverseIndex / 1000f
                MessageItem(
                    message = message,
                    showStatus = showStatus,
                    traversalIndexBase = messageTraversal,
            ...
            // The traversalIndexBase ensures TalkBack reads the date header before its section's messages.
            item(contentType = DateHeader, key = "SectionHeader_$sectionIndex") {
                MessageGroupHeader(
                    dayString = section.createdAtDate,
                    modifier = Modifier.animateItem(),
                    traversalIndexBase = sectionBase
                )
            }
        }

        if (canLoadMore) {
            item(contentType = Loading, key = "LoadMore") {
