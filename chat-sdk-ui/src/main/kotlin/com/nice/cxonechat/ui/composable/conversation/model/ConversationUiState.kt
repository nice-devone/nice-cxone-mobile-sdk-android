...
internal data class ConversationUiState(
    ...
    internal fun messages(context: Context): Flow<List<Section>> = sdkMessages
        .map { messageList ->
            messageList
                ...
                .groupMessages(context.resources)
                .entries
                .map(::Section)
        }
    ...
    private companion object {
        ...
        fun List<Message>.groupMessages(resources: Resources): Map<String, List<Message>> {
            ...
            return drop(1)
                .fold(initial) { groups: MutableList<MutableList<Message>>, curr: Message ->
                    ...
                    if (prev.createdAt.toEpochMilliseconds() - curr.createdAt.toEpochMilliseconds() <= TWO_MINUTES) {
                        lastGroup.add(curr)
                    } else {
                ...
                .associateBy { group -> group.minBy(Message::createdAt).createdAtDate(resources) }
        }
    }
}
