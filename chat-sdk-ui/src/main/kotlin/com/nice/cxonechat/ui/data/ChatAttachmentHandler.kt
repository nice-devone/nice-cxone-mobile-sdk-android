    ...
    fun onAttachmentClicked(activity: Activity, attachment: Attachment) = timedScope("onAttachmentClicked") {
        ...
        when {
            mimeType.startsWith("image/") -> chatThreadViewModel.showImage(
                image = url,
                title = title.takeUnless { it.isNullOrEmpty() }?.let {
                    activity.getString(string.image_preview_title, it)
                } ?: activity.getString(string.image_preview_title_fallback),
            ...
            mimeType.startsWith("video/") -> chatThreadViewModel.showVideo(
                url = url,
                title = title.takeUnless { it.isNullOrEmpty() }?.let {
                    activity.getString(string.video_preview_title, it)
                } ?: activity.getString(string.video_preview_title_fallback),
                attachment = attachment
            )
