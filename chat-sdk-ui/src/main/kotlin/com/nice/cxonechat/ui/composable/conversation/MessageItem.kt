...
internal fun MessageItem(
    ...
    Column(
        modifier = Modifier.semantics {
            ...
            traversalIndex = traversalIndexBase
        }
    ) {
        Row(
            modifier = modifier
...
private fun MessageContentBody(
    ...
    when (message) {
        is Unsupported -> UnsupportedMessage(message, Modifier.padding(space.messagePadding), snackBarHostState)
        ...
        is QuickReply -> QuickReplyMessage(message, messageStatusState, Modifier.padding(space.messagePadding), snackBarHostState)
    }
}
...
private fun MessageFrameContent(
    ...
    MessageFrame(
        ...
        trailingContent = { TrailingContent(message, onShare) }
    ) {
        MessageContentBody(
            message = message,
            modifier = modifier,
...
@Composable
private fun TrailingContent(message: Message, onShare: (Collection<Attachment>) -> Unit) {
    if (message is AttachmentsMessage) {
        ...
        val contentDescription = if (attachmentList.size == 1) {
            stringResource(string.share_attachment, attachmentList[0].mimeTypeToDescription())
        } else {
            stringResource(string.share_attachment_others, attachmentList[0].mimeTypeToDescription(), attachmentList.size - 1)
        }
        ShareAttachmentsIcon(contentDescription) { onShare(attachmentList) }
    }
}
...
private fun PreviewContentTextMessage() {
    ...
    ChatTheme {
        ChatTheme.Scaffold(
            ...
            backgroundColor = chatColors.token.background.default,
