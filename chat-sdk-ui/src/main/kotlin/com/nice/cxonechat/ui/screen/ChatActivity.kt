...
class ChatActivity : ComponentActivity(), AndroidScopeComponent {
    ...
    private var waitForReadyJob: Job? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) = loggerScope.scope("onCreate") {
        ...
        duration {
            ...
            // Enable deobfuscated Compose function names in stack traces for minified host apps.
            // Auto uses GroupKeys when the host app is minified, None otherwise — safe for an SDK.
    ...
    override fun onPause() = loggerScope.scope("onPause") {
        super.onPause()
        duration {
            waitForReadyJob?.cancel()
            waitForReadyJob = null
        }
    }

    override fun onStop(): Unit = loggerScope.scope("onStop") {
        super.onStop()
        lifecycleScope.launch { duration { chatViewModel.close() } }
    }
    ...
    private fun ChatUi(chatState: ChatState, snackbarHostState: SnackbarHostState) {
        ...
        val screenGraph = navController.createGraph(startDestination = initialScreen) {
            ...
            composable<ThreadScreen> {
                ...
                ThreadScreen(snackbarHostState, isMultiThread, isLiveChat, testMod, onBack = { backDispatcher?.onBackPressed() })
            }
        }
        ThreadNotFoundEffect(navController)
        ChatDialogScreen(
            ...
            retryAction = remember {
                {
                    loggerScope.verbose("Refresh chat state after failure")
                    chatViewModel.refreshThreadState()
                }
            },
        )
        ChatStateNavEffect(navController)
        ChatErrorScreen(onTerminalError = ::finish)
        NavHost(navController, screenGraph, modifier = testMod)
    }

    @Composable
    private fun ThreadNotFoundEffect(navController: NavController) {
    ...
    @Composable
    private fun ChatStateNavEffect(navController: NavController) {
        ChatStateEffect(
        ...
        ChatTheme.Scaffold(
            ...
            topBar = {
                ...
                if (dialogState !is ConversationDialog.FullScreenDialog) {
                    ThreadViewTopBar(isMultiThread, isLiveChat, scrollBehavior, onBack)
                }
            },
        ) { padding ->
    ...
    override fun onResume() {
        ...
        waitForReadyJob?.cancel()
        waitForReadyJob = lifecycleScope.launch {
            loggerScope.scope("onResume.waitForReady") {
                verbose("Waiting for chat state to be Ready before refreshing thread state")
                chatStateViewModel.state.first { it === Ready }
                verbose("Chat state is Ready, refreshing thread state")
                chatViewModel.refreshThreadState()
            }
        }
    }
