...
internal open class PluginPreviewProvider: PreviewParameterProvider<Message> {
    data class File(
    ...
    data class Text(
        ...
        override val isMarkdown: Boolean = false,
    ...
    val text: List<Message> = listOf(
        Text("Some Text"),
        Text("Some **bold** text.", isMarkdown = true),
        Text("Some <b>bold</b> text.", isHtml = true)
...
internal fun PluginElement.asMessage(name: String) = Message.Plugin(
    object : Plugin() {
        ...
        override val direction = ToClient
