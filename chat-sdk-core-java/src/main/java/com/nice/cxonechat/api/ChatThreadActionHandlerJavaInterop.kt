...
@file:JvmName("ChatThreadActionHandlerJavaInterop")
...
/**
 * Java-friendly observer for [ChatThreadActionHandler.popupFlow].
 *
 ...
 *
 * @param handler The [ChatThreadActionHandler] whose popups to observe.
 * @param listener Invoked for each [Popup] emission.
...
@JvmOverloads
fun onPopup(
    handler: ChatThreadActionHandler,
    listener: Consumer<Popup>,
    onError: ErrorCallback? = null,
    callbackExecutor: Executor? = null,
): Cancellable = launchCancellable(callbackExecutor) { collectThreadPopupFlow(handler, listener, onError) }

@VisibleForTesting
@JvmSynthetic
internal suspend fun collectThreadPopupFlow(
    handler: ChatThreadActionHandler,
    listener: Consumer<Popup>,
    onError: ErrorCallback? = null,
) {
