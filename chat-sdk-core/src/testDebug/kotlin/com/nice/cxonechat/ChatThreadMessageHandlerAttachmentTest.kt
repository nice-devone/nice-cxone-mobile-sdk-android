...
internal class ChatThreadMessageHandlerAttachmentTest : AbstractMultiThreadChatTest() {
    ...
    @Test
    fun send_too_large_attachment() = runTest {
        val bytes = Random.nextBytes(FILE_SIZE * 1024 * 1024 + 1)
        val filename = "file_too_large"
        val upload = contentDescriptor(bytes, filename)
        val runtimeExceptionCount = chatStateListener.onChatRuntimeExceptions.size

        assertFailsWith<AttachmentUploadError> {
            messages.send(OutboundMessage(listOf(upload), ""))
        }

        assertTrue(chatStateListener.onChatRuntimeExceptions.size > runtimeExceptionCount)
        assertIs<AttachmentUploadError>(chatStateListener.onChatRuntimeExceptions.last())
        verify(exactly = 0) { service.uploadFile(any(), any(), any()) }
    }

    @Test
    fun send_attachment_without_allowed_type() = runTest {
    ...
    @Test
    fun send_attachment_when_disabled_via_config() = runTest {
