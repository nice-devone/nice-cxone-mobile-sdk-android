...
class ChatThreadEventHandlerTokenGuardTest {
    ...
    private val tokenGuard = ChatThreadEventHandlerTokenGuard(mockOriginHandler, mockChat, mockCoordinator)
    ...
    @Test
    fun `trigger handles null token expiration date`() = runTest {
    ...
    @Test
    fun `trigger with expired token - past expiration`() = runTest {
        // Token expired 10 seconds ago
        every { mockStorage.authTokenExpDate } returns
                Clock.System.now() - 10.seconds
    ...
    @Test
    fun `trigger delegates delegation correctly to ChatThreadEventHandler`() = runTest {
        every { mockStorage.authTokenExpDate } returns Instant.DISTANT_FUTURE
