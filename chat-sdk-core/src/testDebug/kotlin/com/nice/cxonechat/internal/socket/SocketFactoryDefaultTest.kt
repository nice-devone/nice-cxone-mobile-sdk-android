...
class SocketFactoryDefaultTest {
    ...
    @Test
    fun `create with transactionToken appends token to url`() {
        val transactionToken = "test-token"
        val webSocket: WebSocket = mockk()
        val requestSlot = slot<Request>()
        val client: OkHttpClient = mockk {
            every { newWebSocket(capture(requestSlot), any()) } returns webSocket
        }
        val builder: OkHttpClient.Builder = mockk {
            every { connectTimeout(any(), any()) } returns this
            every { pingInterval(any(), any()) } returns this
        ...
        socketFactory.create(mockk(), visitorId, transactionToken)

        requestSlot.captured.url.query shouldBe buildString {
            append("brandId=")
            append(brandId)
            append("&channelId=")
            append(channelId)
    ...
    @Test
    fun `createProxyListener returns a ProxyWebSocketListener`() {
        val socketFactory = SocketFactoryDefault(configuration, mockk())
        assertIs<ProxyWebSocketListener>(socketFactory.createProxyListener())
    }
