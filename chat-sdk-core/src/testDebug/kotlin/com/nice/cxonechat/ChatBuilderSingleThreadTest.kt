...
/**
 * Tests for [ChatBuilder] connect behavior specific to SingleThread mode.
 * In SingleThread mode, after authorization the SDK sends [ServerRequest.RecoverThread]
 * to restore the existing thread (no [ServerRequest.FetchThreadList]).
 */
internal class ChatBuilderSingleThreadTest : AbstractChatTestSubstrate() {
    ...
    @Test
    fun `connect sends RecoverThread`() = runTest(dispatcher) {
        val code = "code"
        val verifier = "verifier"
        val (connection, builder) = prepareBuilder()
        // AuthorizeConsumer is the first message; RecoverThread follows (no FetchThreadList)
        assertSendTexts(
    ...
    private fun connect(
        ...
        // Drive SingleThread recovery: ChatSingleThread.connect() suspends waiting for this
