...
internal object ServerResponse {
                        ...
                        val handover = object {
                            val customFields = fields.map { (key, value) ->
                                object {
                                    val ident = key
                                    val value = value
                                    val updatedAt = Instant.fromEpochMilliseconds(0)
                                }
                            }
                        }
                    }
    ...
    fun MessageReadChanged(
        ...
        val data = object {
            val message = message.copy(
                userStatistics = message.userStatistics.copy(readAt = Instant.fromEpochMilliseconds(0))
            )
        }
    }.serialize()
    ...
    fun CaseStatusChanged(
        ...
        val data = object {
            val case = object {
                val threadIdOnExternalPlatform = thread.id
                val status = status
                val statusUpdatedAt = Instant.fromEpochMilliseconds(0)
            }
        }
    }.serialize()
    ...
    fun EventInS3(
        ...
        eventType: EventType = EventType.MessageCreated,
    ) = object {
        val eventId = TestUUID
    ...
    fun LivechatRecovered(
        ...
        thread: ChatThread = makeChatThread(),
        agent: AgentModel? = makeAgent(),
        previousInboxAssignee: AgentModel? = null,
        ownerAssignee: AgentModel? = null,
        customerCustomFields: List<CustomField> = emptyList(),
        ...
        val postback = object {
            ...
            val data = object {
                ...
                val messages = messages
                val inboxAssignee = agent
                val previousInboxAssignee = previousInboxAssignee
                val ownerAssignee = ownerAssignee
                val thread = thread.toReceived()
                val messagesScrollToken = scrollToken
            }
        }
                        ...
                        val elements = listOf(
                            ...
                            object {
                                ...
                                val variables = object {
                                    val startedAt = Instant.fromEpochMilliseconds(0)
