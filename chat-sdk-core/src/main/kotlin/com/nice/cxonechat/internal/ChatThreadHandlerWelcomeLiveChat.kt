...
internal class ChatThreadHandlerWelcomeLiveChat(
    ...
    init {
        scope("init") {
            ...
            // Register lateAppend with the thread lifecycle so socket callbacks are cleaned up
            // if the handler is discarded before the welcome message is ever sent.
            mutableThread.resultCallbacks[UUIDProvider.next()] = Cancellable { lateAppend.cancel() }
            if (canSendWelcomeEvent(mutableThread.messages.size)) {
                chat.entrails.threading.coroutineScope.safeLaunch(childScope("welcomeLiveChatMonitor")) {
                    origin.threadFlow.safeCollect(childScope("welcomeLiveChatCollect")) { threadUpdate ->
                        scope("ThreadUpdate") {
                            verbose("On thread update")
                            if (canSendWelcomeEvent(threadUpdate.messages.size)) {
                                val storedMessage = storedMessage()
                                val hasStart = threadUpdate.messages.hasConversationStart()
                                if (storedMessage.isNotBlank() && hasStart && eventSendEnabled.getAndSet(false)) {
                    ...
                    mutableThread.resultCallbacks[UUIDProvider.next()] = Cancellable { monitorJob.cancel() }
                }
            }
        }
    }
    ...
    private inner class WelcomeThreadMessageHandlerLiveChat(
        ...
        override suspend fun send(message: OutboundMessage): String = scope("send") {
            ...
            if (canSendWelcome && storedMessage.isNotBlank()) {
                ...
                return addWelcomeMessageAndSend(message = message, storedMessage = storedMessage)
            } else {
                verbose("Sending message without welcome message")
                return originHandler.send(message)
            }
        }
         ...
         */
        private suspend fun addWelcomeMessageAndSend(message: OutboundMessage, storedMessage: String): String =
            scope("addWelcomeMessageAndSend") {
                if (message.message == BEGIN_CONVERSATION_MESSAGE) {
                    // Send begin conversation first, trigger welcome message after sent
                    ...
                    if (eventSendEnabled.getAndSet(false)) {
                        ...
                        safeTriggerWelcomeMessageEvent(storedMessage)
                    }
                    return id
        ...
        private fun safeTriggerWelcomeMessageEvent(storedMessage: String) {
            try {
                triggerWelcomeMessageEvent(storedMessage)
            } catch (expected: CancellationException) {
                throw expected
            } catch (expected: Exception) {
                warning("Welcome message event failed", expected)
            }
        }
    }
    ...
    private fun triggerWelcomeMessageEvent(
        ...
        chat.entrails.threading.coroutineScope.safeLaunch(this) {
            try {
                events().trigger(prepareEvent(message))
            } finally {
                onComplete?.invoke()
            }
        }
