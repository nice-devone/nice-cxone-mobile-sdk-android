...
internal class ChatFieldHandlerThread(
...
) : ChatFieldHandler, LoggerScope by LoggerScope("ChatFieldHandlerThread", parentScope) {
    ...
    override fun add(fields: Map<String, String>): Unit = scope("add") {
        ...
        when (thread.threadState) {
            Received, Loaded, Ready -> coroutineScope.safeLaunch(childScope("triggerSetContactCustomFieldEvent")) {
                handler.events().trigger(SetContactCustomFieldEvent(customFields))
                updateContactCustomFields(customFields)
            }
