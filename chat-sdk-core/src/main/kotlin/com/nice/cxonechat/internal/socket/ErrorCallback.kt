...
internal abstract class ErrorCallback(
    ...
    internal companion object {
        ...
        /**
         * Creates a [Flow] that emits the [WebSocket] instance whenever an [ErrorModel] with a matching
         * [ErrorModel.Error.errorCode] is received. Messages that are not errors or have a non-matching
         * error code are silently ignored.
         *
         * @param errorType The [ErrorType] to filter for.
         * @return A [Flow] of [WebSocket] instances for each matching error.
         */
        internal fun ProxyWebSocketListener.errorFlow(
            errorType: ErrorType,
        ): Flow<WebSocket> = messageFlow.mapNotNull { (webSocket, text) ->
            val errorMessage: ErrorModel? = serializer.runCatching {
                decodeFromString<ErrorModel>(text)
            }.getOrNull()
            if (errorMessage?.error?.errorCode == errorType) {
                webSocket
            } else {
