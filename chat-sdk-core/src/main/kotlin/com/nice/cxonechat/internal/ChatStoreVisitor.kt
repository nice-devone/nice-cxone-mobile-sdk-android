...
internal class ChatStoreVisitor(
    ...
    CoroutineScope by origin.entrails.threading.coroutineScope,
    LoggerScope by LoggerScope("ChatStoreVisitor", origin.entrails.logger) {

    private val retryApiHandler = RetryApiHandler(maxRetries = 2, retryIntervalMs = 30_000L)

    init {
        safeLaunch(childScope("sendVisitorInfo")) {
            sendVisitorInfo()
        }
    }

    private suspend fun sendVisitorInfo() {
