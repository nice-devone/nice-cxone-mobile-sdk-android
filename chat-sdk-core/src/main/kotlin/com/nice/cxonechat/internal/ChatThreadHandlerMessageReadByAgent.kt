...
/**
 * Wraps origin [ChatThreadHandler] and emits via [threadFlow] when [EventMessageReadByAgent]
 * is received. Always causes an update of a message.
 */
...
) : ChatThreadHandler by origin, LoggerScope by LoggerScope("ChatThreadHandlerMessageReadByAgent", chat.entrails.logger) {

    override val threadFlow: Flow<ChatThread> = merge(
        origin.threadFlow,
        chat.socketListener.eventFlow(EventMessageReadByAgent)
            .mapNotNull { (_, event) ->
                ...
                try {
                    ...
                    warning("Failed to handle message-read-by-agent event in threadFlow", expected)
                    null
                }
            },
    )
