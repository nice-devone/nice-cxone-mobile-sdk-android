...
) : ChatThreadHandler by origin, LoggerScope by LoggerScope("ChatThreadHandlerMessages", chat.entrails.logger) {

    override val threadFlow: Flow<ChatThread> = merge(
        origin.threadFlow,
        chat.socketListener.eventFlow(EventMoreMessagesLoaded)
            .mapNotNull { (_, event) ->
                ...
                try {
                    ...
                    warning("Failed to handle more-messages-loaded event in threadFlow", expected)
                    null
                }
            },
        chat.socketListener.eventFlow(EventMessageCreated)
            .mapNotNull { (_, event) ->
                ...
                try {
                    ...
                    warning("Failed to handle message-created event in threadFlow", expected)
                    null
                }
            },
    )
