...
internal class ChatThreadMessageHandlerAttachmentVerification(
    ...
    override suspend fun send(message: OutboundMessage): String {
        return when (val validationResult = validateAttachments(message)) {
            is ValidationResult.Success -> origin.send(message)
            is ValidationResult.Failure -> {
                validationResult.invalidAttachments.forEach(this::notifyListener)
                val first = validationResult.invalidAttachments.first()
                throw AttachmentUploadError(first.filename, InvalidStateException(first.error))
            }
        }
    }
