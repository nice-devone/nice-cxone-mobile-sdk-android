...
internal class ChatBuilderLogging(
...
) : ChatBuilder, ChatBuilderInternal, LoggerScope by LoggerScope<ChatBuilder>(entrails.logger) {

    private var developmentMode: Boolean = false

    override fun setPreferCachedConfiguration(prefer: Boolean): Unit = scope("setPreferCachedConfiguration") {
        val internal = origin as? ChatBuilderInternal
        if (internal == null) {
            warning("Origin builder is not ChatBuilderInternal; ignoring setPreferCachedConfiguration($prefer)")
        } else {
            internal.setPreferCachedConfiguration(prefer)
        }
    }
    ...
    override fun setTokenDelegateListener(listener: TokenDelegateListener): ChatBuilder = scope("setTokenDelegateListener") {
        origin.setTokenDelegateListener(listener)
    }

    override suspend fun build(): Chat = scope("build") {
        return try {
            duration { origin.build() }
