...
internal object CookieAsStringSerializer : KSerializer<Cookie> {
    ...
    override val descriptor =
        PrimitiveSerialDescriptor("Cookie", STRING)
...
private class EncryptedCookieListSerializer(
...
) : Serializer<List<Cookie>>, LoggerScope by LoggerScope("CookieListSerializer", logger) {
    ...
    override suspend fun readFrom(input: InputStream): List<Cookie> {
        return runCatching {
            ...
            val decrypted = encryption.decrypt(encrypted, null).decodeToString()
            json.decodeFromString(listSerializer, decrypted)
        }.onFailure { e ->
            error("Unexpected error while reading cookie data", e)
        }.getOrNull() ?: emptyList()
    }
    ...
    override suspend fun writeTo(
        ...
        runCatching {
            ...
            val encrypted = encryption.encrypt(encoded.toByteArray(), null)
            output.write(encrypted)
        }.onFailure { e ->
            error("Failed to write encrypted cookie data", e)
        }
    }
}
...
internal class EncryptedCookieDataStore private constructor(
    ...
    suspend fun loadAllCookies(): List<Cookie> = dataStore.data.first()
    ...
    companion object {
        ...
        suspend fun create(
            ...
            withContext(Dispatchers.IO) {
                ...
                val encryption: Aead = keysetHandle.getPrimitive(
                    RegistryConfiguration.get(),
                    Aead::class.java
                )

                val dataStore = DataStoreFactory.create(
                    serializer = EncryptedCookieListSerializer(encryption, logger),
