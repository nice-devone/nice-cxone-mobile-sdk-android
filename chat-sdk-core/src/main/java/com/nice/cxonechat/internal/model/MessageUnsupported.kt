...
internal data class MessageUnsupported(
    private val model: MessageModel,
    private val content: MessagePolyContent.Unsupported,
) : Message.Unsupported() {
    ...
    override val text: String
        get() = buildString {
            appendLine(fallbackText)
            append(content.type.uppercase())
            content.specificType?.let {
                append(" — ")
                append(it.uppercase())
            }
        }
